...
def read_flow_csv(file_path: Path) -> pd.DataFrame:
    if re.fullmatch(r"UNSW-NB15_\d+\.csv", file_path.name, flags=re.IGNORECASE):
        return pd.read_csv(file_path, header=None, names=UNSW_RAW_COLUMNS, low_memory=False)
...
def summarize_windows(flows: pd.DataFrame) -> pd.DataFrame:
    summary = (
        flows.groupby("window_start")["binary_label"]
        .agg(malicious_flows="sum", total_flows="count")
        .reset_index()
        .sort_values("window_start")
        .reset_index(drop=True)
    )
    summary["malicious_flows"] = summary["malicious_flows"].astype(int)
    summary["total_flows"] = summary["total_flows"].astype(int)
    summary["has_attack"] = (summary["malicious_flows"] > 0).astype(int)
    summary["malicious_ratio"] = summary["malicious_flows"] / summary["total_flows"].clip(lower=1)
    return summary
...
def assign_stratified_splits(
    ...
    required = {"window_start", "has_attack"}
    missing = required.difference(window_summary.columns)
    if missing:
        raise ValueError(f"window_summary is missing required columns: {sorted(missing)}")

    train_r, val_r, test_r = split_ratio
    windows = window_summary["window_start"].tolist()
    labels = window_summary["has_attack"].tolist()
    n = len(windows)
    if n == 0:
        return {}

    train_count = int(n * train_r)
    val_end = int(n * (train_r + val_r))
    val_count = val_end - train_count
    test_count = n - train_count - val_count

    if n < 3 or min(labels.count(0), labels.count(1)) < 2 or min(train_count, val_count, test_count) == 0:
        LOGGER.warning("Falling back to temporal split because dataset is too small for stable stratification.")
        return assign_temporal_splits(windows, split_ratio)
    ...
    split_map: Dict[pd.Timestamp, str] = {}
    for window_start in train_windows:
        split_map[window_start] = "train"
    for window_start in val_windows:
        split_map[window_start] = "val"
    for window_start in test_windows:
        split_map[window_start] = "test"

    LOGGER.info(
        "Assigned %s windows using stratified split: train=%s val=%s test=%s",
        n,
        len(train_windows),
        len(val_windows),
        len(test_windows),
    ...
    LOGGER.info(
        "Attack-window counts by split after stratification: train=%s val=%s test=%s",
        int(sum(train_labels)),
        int(sum(val_labels)),
        int(sum(test_labels)),
    )
...
def run_pipeline(args: argparse.Namespace) -> Dict[str, object]:
    ...
    window_summary = summarize_windows(flows)
    windows = window_summary["window_start"].tolist()
    if args.split_strategy == "temporal":
        split_map = assign_temporal_splits(windows, split_ratio)
    elif args.split_strategy == "stratified_attack_presence":
        split_map = assign_stratified_splits(window_summary, split_ratio, seed=args.seed)
    else:
    ...
    window_split_series = window_summary["window_start"].map(split_map)
    window_attack_distribution_by_split = {}
    for split_name in ["train", "val", "test"]:
        split_mask = window_split_series.eq(split_name)
        window_attack_distribution_by_split[split_name] = {
            "attack_windows": int((split_mask & window_summary["has_attack"].eq(1)).sum()),
            "benign_only_windows": int((split_mask & window_summary["has_attack"].eq(0)).sum()),
        }
    ...
    manifest = {
        ...
        "summary": {
            ...
            "window_attack_distribution": {
                "overall": {
                    "attack_windows": int(window_summary["has_attack"].sum()),
                    "benign_only_windows": int((window_summary["has_attack"] == 0).sum()),
                },
                "by_split": window_attack_distribution_by_split,
            },
            "feature_dims": {
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--split-strategy",
        default="temporal",
        choices=["temporal", "stratified_attack_presence"],
        help="How to assign windows to train/val/test.",
    )
    parser.add_argument(
        "--max-rows-per-file",
        type=int,
